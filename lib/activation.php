...
function roots_theme_activation_options_render_page() { ?>
  <div class="wrap">
    <h2><?php printf(__('%s Theme Activation', 'stoker'), wp_get_theme()); ?></h2>
    <div class="update-nag">
      <?php _e('These settings are optional and should usually be used only on a fresh installation', 'stoker'); ?>
    </div>
    <?php settings_errors(); ?>
    ...
    <form method="post" action="options.php">
      ...
      <table class="form-table">
        <tr valign="top"><th scope="row"><?php _e('Create static front page?', 'stoker'); ?></th>
          <td>
            <fieldset>
              <legend class="screen-reader-text"><span><?php _e('Create static front page?', 'stoker'); ?></span></legend>
              <select name="roots_theme_activation_options[create_front_page]" id="create_front_page">
                <option selected="selected" value="true"><?php echo _e('Yes', 'stoker'); ?></option>
                <option value="false"><?php echo _e('No', 'stoker'); ?></option>
              </select>
              <p class="description"><?php printf(__('Create a page called Home and set it to be the static front page', 'stoker')); ?></p>
            </fieldset>
          </td>
        </tr>
        <tr valign="top"><th scope="row"><?php _e('Change permalink structure?', 'stoker'); ?></th>
          <td>
            <fieldset>
              <legend class="screen-reader-text"><span><?php _e('Update permalink structure?', 'stoker'); ?></span></legend>
              <select name="roots_theme_activation_options[change_permalink_structure]" id="change_permalink_structure">
                <option selected="selected" value="true"><?php echo _e('Yes', 'stoker'); ?></option>
                <option value="false"><?php echo _e('No', 'stoker'); ?></option>
              </select>
              <p class="description"><?php printf(__('Change permalink structure to /&#37;postname&#37;/', 'stoker')); ?></p>
            </fieldset>
          </td>
        </tr>
        <tr valign="top"><th scope="row"><?php _e('Create navigation menu?', 'stoker'); ?></th>
          <td>
            <fieldset>
              <legend class="screen-reader-text"><span><?php _e('Create navigation menu?', 'stoker'); ?></span></legend>
              <select name="roots_theme_activation_options[create_navigation_menus]" id="create_navigation_menus">
                <option selected="selected" value="true"><?php echo _e('Yes', 'stoker'); ?></option>
                <option value="false"><?php echo _e('No', 'stoker'); ?></option>
              </select>
              <p class="description"><?php printf(__('Create the Primary Navigation menu and set the location', 'stoker')); ?></p>
            </fieldset>
          </td>
        </tr>
        <tr valign="top"><th scope="row"><?php _e('Add pages to menu?', 'stoker'); ?></th>
          <td>
            <fieldset>
              <legend class="screen-reader-text"><span><?php _e('Add pages to menu?', 'stoker'); ?></span></legend>
              <select name="roots_theme_activation_options[add_pages_to_primary_navigation]" id="add_pages_to_primary_navigation">
                <option selected="selected" value="true"><?php echo _e('Yes', 'stoker'); ?></option>
                <option value="false"><?php echo _e('No', 'stoker'); ?></option>
              </select>
              <p class="description"><?php printf(__('Add all current published pages to the Primary Navigation', 'stoker')); ?></p>
            </fieldset>
          </td>
        </tr>
      </table>
...
function roots_theme_activation_action() {
  ...
  if ($roots_theme_activation_options['create_front_page'] === 'true') {
    ...
    $default_pages = array(__('Home', 'stoker'));
    ...
    $home = get_page_by_title(__('Home', 'stoker'));
  ...
  if ($roots_theme_activation_options['create_navigation_menus'] === 'true') {
    ...
    $primary_nav = wp_get_nav_menu_object(__('Primary Navigation', 'stoker'));

    if (!$primary_nav) {
      $primary_nav_id = wp_create_nav_menu(__('Primary Navigation', 'stoker'), array('slug' => 'primary_navigation'));
